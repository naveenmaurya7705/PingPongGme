...
function set_interval() {
  ...
  if (dimension_ball.bottom < 0 || dimension_ball.top > window.innerHeight) {
    ...
    let Local_Value1 = parseInt(localStorage.getItem("Player1_max_score"));
    let Local_Value2 = parseInt(localStorage.getItem("Player2_max_score"));

    let Find_max_rod1 = Math.max(Local_Value1, rod1_Score).toString();
    let Find_max_rod2 = Math.max(Local_Value2, rod2_Score).toString();
    localStorage.setItem("Player1_max_score", Find_max_rod1);
    localStorage.setItem("Player2_max_score", Find_max_rod2);
    gameStart = false;
